...
from backbones.base import ExtendedModule
from backbones.counting import CountedAdd, CountedEinsum
...
class RelativePositionEmbedding(ExtendedModule):
    ...
    def forward(self, x, q, inplace=True):
        ...
        if x.shape[-1] != (self.attention_size[0] * self.attention_size[1]):
            n_tokens = int(math.sqrt(x.shape[-1]))
            a = (n_tokens, n_tokens)
        else:
            a = self.attention_size
