...
from backbones.backbones import ViTBackbone
from backbones.base import ExtendedModule, numeric_tuple
from backbones.blocks import LN_EPS
from utils.image import as_float32, pad_to_size
from functools import partial
from timm.models.vision_transformer import VisionTransformer
from backbones.backbone_vit import vit_base_patch16
from backbones.backbone_windowed import ViTBackbone as MaskedViTBackbone
...
class ViTDet(ExtendedModule):
    ...
    def __init__(
        ...
        if backbone_config["backbone"] == "windowed":
            if mask:
                self.backbone = MaskedViTBackbone(
            ...
            self.backbone_embedding = False
        elif backbone_config["backbone"] == "non-windowed":
            self.backbone = vit_base_patch16(img_size=672, patch_size=16)
            self.backbone_embedding = True
        else:
            assert "Backbone not defined"
    ...
    def forward(self, x, mask_index=None):
        ...
        if mask_index is not None:
            x = self.backbone(x, mask_id=mask_index)
        else:
            x = self.backbone(x)
        # x = self.backbone.get_intermediate_layers(x, 1)[0] # get output of last layer 
        results, losses = self.post_backbone(images, x)
    ...
    def post_backbone(self, images, x):
        ...
        result, losses = self.roi_heads(images, x, proposals, None)
        result = [
    ...
    def pre_backbone(self, x):
        ...
        images = ImageList.from_tensors([image for image in x])
        if not self.backbone_embedding:
            x = self.embedding(x)
        return images, x
